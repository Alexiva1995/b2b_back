    ...
    public function levelOne()
    {
        ...
        foreach ($users as $user) {
            foreach ($user->marketPurchased as $matrixPurchased) {
                $userRight = $users->where('father_cyborg_purchased_id', $matrixPurchased->id)->where('binary_side', 'R')->where('buyer_id', $user->id)->where('status', User::ACTIVE)->whereHas('marketPurchased', function ($q, $matrixPurchased) {
                    $q->where('type', '>=', $matrixPurchased->type);
                })->first();
                $userLeft = $users->where('father_cyborg_purchased_id', $matrixPurchased->id)->where('binary_side', 'L')->where('buyer_id', $user->id)->where('status', User::ACTIVE)->whereHas('marketPurchased', function ($q, $matrixPurchased) {
                    $q->where('type', '>=', $matrixPurchased->type);
                })->first();
